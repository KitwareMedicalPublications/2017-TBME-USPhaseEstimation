...
%
\caption{\rk{Evaluation of the quality of single-cycle videos reconstructed using our kernel regression model at 5 increasing levels of temporal magnification. We down-sampled each of the 6 videos in our dataset by increasing factors between 1x-5x, reconstructed the single-cycle video back at the original resolution using our kernel regression model, and computed its similarity using normalized correlation (ncorr) to the ground truth single cycle video created by cropping out the portion between two ECG R-wave peaks where respiratory motion is minimal. We created the down-sampled videos starting at 7 random points in the first 14 frames to avoid any bias to the starting phase and computed the mean and stddev of the ncorr between the reconstructed and ground truth single cycle video. As a baseline, we show the mean ncorr among all consecutive frames (yellow dashed lines) and the min ncorr among all pairs of frames (green dashed lines) in the ground truth video.}}
\label{fig:single_cycle_video_recon}
\end{figure}

\rk{The supplementary material includes videos showing the instantaneous cardiac and respiratory phases estimated using the proposed phase-estimation method and the single-cycle videos at 1x, 2x, 4x, and 8x temporal magnification generated using the proposed kernel regression model for one of the 6 videos in our dataset}. 
